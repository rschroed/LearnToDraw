...
V2 `DrawingSession` records begin from intent alone. Creation persists `planning` state and dispatches provider work without moving hardware. The drawing advisor returns a concise plan, paper strategy, completion intent, ordered creative criteria, and first-pass SVG. Criteria rank the qualities that make the request distinctive; generic subject recognition and plot safety are constraints rather than substitutes for the requested medium, technique, mood, scientific character, or composition.

The initial SVG is untrusted. The backend validates its exact drawable-area canvas, supported passive elements, direct coordinates, safe stroke styling, and in-bounds geometry before rendering a transient black-on-white page image with `resvg_py`. A separate provider call reviews that image criterion by criterion. It may accept the candidate or replace it once with a complete revised SVG; a replacement passes the same validator before storage. Only the selected safe asset can be exposed in `awaiting_approval`. The approval screen shows both the ranked criteria and review result. Review rasters are not persisted as plot assets, and this bounded two-call planning path never receives hardware access or moves the plotter.
...
After a registered observation completes, a single backend coordinator sends the rectified grayscale page, current plan, ranked creative criteria, bounded interpretation history, and atomically consumed guidance through `assess_iteration`. The advisor records one finding per criterion and must judge the visible page instead of reinforcing its earlier assessment. `continue` requires a validated incremental SVG that addresses a named criterion gap and creates exactly one next normal PlotRun. `complete` ends the session. A pause decision must classify its recovery as `retake_capture` or `replan_new_sheet`; provider and attendance failures use the separate resumable `resume` action. This prevents a generic resume from assessing the same immutable observation again.

One additive V2 session represents one physical sheet. When the advisor decides that an irreversible foundation cannot be repaired additively, `replan_new_sheet` atomically marks the original session abandoned, preserves its artifacts and timeline, and creates exactly one linked planning successor. The successor receives the prior assessment and criterion findings as planning context but receives no authorization, PlotRun, or inherited pass count. Planning may begin immediately because it cannot move hardware; plotting still requires the normal explicit blank-paper, orientation, and pen confirmation. `POST /api/drawing-sessions/{id}/replan` exposes the same idempotent transition for older paused sessions whose agent decision predates the typed recovery field.
...
Paused capture failures and questionable frames awaiting manual registration offer a camera-only retake, which cannot create a replacement plot run. Manual registration remains in-context on the canvas after the replacement capture. Resumable provider, attendance, and restart pauses expose Resume, while irreparable agent decisions expose a linked new-sheet attempt instead. The studio follows an agent-created successor automatically, explains what carried forward, and never presents a stale-observation Resume action.
...
Advisor credentials remain a backend-owned secret. Controls may submit an API key to the loopback-only configuration endpoint, but the frontend clears the password field after success and never writes the key to browser storage, artifacts, URLs, or responses. While an OpenAI advisor is active, `PATCH /api/drawing-advisor/configuration` can replace only its model; the backend constructs a new delegate with the credential it already owns and never returns that credential to the browser. A thread-safe snapshot ensures work already in flight finishes with its original delegate while future work uses the replacement. Runtime configuration exists only in backend process memory and clear/restart restores the startup environment configuration.

OpenAI planning and assessment use the Responses API with structured output and `store: false`. Requests run inside the existing asynchronous session workers with a bounded 180-second HTTP timeout; an expiry pauses the session with explicit retry-or-change-model guidance. The Responses API also supports background execution, but that would change response storage and polling semantics and is not part of the current local orchestration contract. [Official Responses API reference](https://developers.openai.com/api/reference/cli/resources/responses/methods/create)
