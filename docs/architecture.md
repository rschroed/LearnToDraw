...
After a registered observation completes, a single backend coordinator sends the rectified grayscale page, current plan, ranked creative criteria, bounded interpretation history, and atomically consumed guidance through `assess_iteration`. The advisor records one finding per criterion and must judge the visible page instead of reinforcing its earlier assessment. `continue` requires a validated incremental SVG that addresses a named criterion gap and creates exactly one next normal PlotRun. `complete` ends the session. A foundational creative mismatch that cannot be repaired additively must produce `pause`, as must a requested human action. Provider, hardware, registration, and artifact failures pause rather than permitting another physical pass.
